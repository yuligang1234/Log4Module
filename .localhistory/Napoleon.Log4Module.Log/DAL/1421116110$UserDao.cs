...
namespace Napoleon.Log4Module.Log.DAL
{
    public static class UserDao
    {
        ...
        /// <param name="starTime">page</param>
        /// <param name="endTime">page</param>
        /// Author  : 俞立钢
        /// Company : 绍兴标点电子技术有限公司
        /// Created : 2015-01-13 09:37:54
        public static List<SystemLog> SelectLog(this SystemLog log, int rows, int page, DateTime starTime, DateTime endTime)
        {
            List<SystemLog> logs = new List<SystemLog>();
            string sql = string.Format("SELECT Id,UserName,IpAddress,OperateTime,OperateType,OperateUrl,OperateContent FROM dbo.System_Log where UserName like '{0}' and IpAddress like '{1}' and OperateType like '{2}' and OperateUrl like '{3}' and OperateContent like '{4}'", log.UserName, log.IpAddress, log.OperateType, log.OperateUrl, log.OperateContent);
            if()
            return logs;
        }
