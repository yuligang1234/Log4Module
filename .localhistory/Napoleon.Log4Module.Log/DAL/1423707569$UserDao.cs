...
namespace Napoleon.Log4Module.Log.DAL
{
    public static class UserDao
    {
        ...
        {
            string sql = string.Format("Insert into System_Log(UserName,IpAddress,OperateTime,OperateType,OperateUrl,OperateContent) values(@UserName,@IpAddress,@OperateTime,@OperateType,@OperateUrl,@OperateContent)", log.UserName, log.IpAddress, log.OperateTime, log.OperateType, log.OperateUrl, log.OperateContent);
        ...
        {
            string sql = string.Format("SELECT count(*) FROM dbo.System_Log where UserName like '%{0}' and IpAddress like '%{1}' and OperateType like '%{2}' and OperateUrl like '%{3}' and OperateContent like '%{4}' and OperateTime>'{5}' and OperateTime<'{6}'", log.UserName, log.IpAddress, log.OperateType, log.OperateUrl, log.OperateContent, startTime, endTime);
            DataTable dt = SqlHelper.GetDataTable(sql);
            if (dt.Rows.Count > 0)
            {
        ...
        {
            string sql = string.Format("SELECT Id,UserName,IpAddress,OperateTime,OperateType,OperateUrl,OperateContent FROM dbo.System_Log where UserName like '%{0}' and IpAddress like '%{1}' and OperateType like '%{2}' and OperateUrl like '%{3}' and OperateContent like '%{4}' and OperateTime>'{5}' and OperateTime<'{6}' ", log.UserName, log.IpAddress, log.OperateType, log.OperateUrl, log.OperateContent, startTime, endTime);
            DataTable dt = SqlHelper.GetDataTable(sql);
            return dt;
        }
        ...
        {
            string sql = string.Format("select Id,UserName,IpAddress,OperateTime,OperateType,OperateUrl,OperateContent FROM dbo.System_Log where Id='{0}'", id);
            SystemLog log = SqlHelper.GetEnumerable<SystemLog>(sql);
            return log;
        }
